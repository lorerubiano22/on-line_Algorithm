...
public class InsertionAlgorithm_go_back {
	private Test aTest;
	...
	public InsertionAlgorithm_go_back(Test myTest, Inputs myInputs) {
		aTest = myTest;
		inputs = myInputs;
	...
	public InsertionAlgorithm_go_back(Test myTest, Inputs myInputs, Solution solution) {
		aTest = myTest;
		inputs = myInputs;
		sol=solution;
		temporing=0;
	...
	public Solution solveMe() {
		...
		//if(exploring()) {
			...
			if(!visted_EdgesList.isEmpty()) {
				/*If the last visited edge is disrupted retorn to the las active edge*/
				if(visted_EdgesList.get((visted_EdgesList.size()-1)).getState()==0) {
					if(visted_EdgesList.size()==1) {
						prevEnd=visted_EdgesList.get((visted_EdgesList.size()-1)).getEnd();
						finalEdge=Optimization.inputs.getedge(prevEnd.getId(), depotEnd.getId());
						visted_EdgesList.add(finalEdge);
					}
					else{
						if(sol.getRoutes().getFirst().getLastconnections()!=-1) {
						prevEnd=visted_EdgesList.get((visted_EdgesList.size()-1)).getEnd();
						finalEdge=Optimization.inputs.getedge(prevEnd.getId(), visted_EdgesList.get((visted_EdgesList.size()-2)).getEnd().getId());	
						visted_EdgesList.add(finalEdge);
					}
				}
				}
	...
	public void doInsertion(Node startRoute, Node endRoute, double operationTime) {
		...
		if(notVisitedVictims.size()!=0) {
			...
			/*** Strategy I: Selecting patients directly connected***/
			AdjEdges=patientDepotEdges(startRoute,endRoute,disruption);
			if(AdjEdges.size()==0) {
				AdjEdges=getEdges(startRoute,endRoute,disruption);
			}
			if(AdjEdges.size()==0) {
				disruption=true;}
			if(AdjEdges.size()!=0) {
				/*Sorting the list of promissing edges to insert*/
				sortingAdj(AdjEdges,disruption);
				boolean insertion=true;
				if( notUsedNodes.size()!=0) {
					//if(notUsedEdges.size()!=0 || notUsedNodes.size()!=0) {
					//if(notUsedNodes.size()!=0) {
					for(Edge adj:AdjEdges) {
						//si el arco ya ha sido visitado no lo visito
						if(isVisited(adj)) {
							insertion=false;
						}
						else {
							// si el arco no ha sido visitado
							/*solo selecciono los nodos dif a los depots*/
							if(adj.getEnd().getId()!=0 && adj.getEnd().getId()!=0) {
								//if(startRoute.getId()!=adj.getOrigin().getId())
								{
									double time=adj.getTime();
									/*Finding the edge at the end of the routes*/
									Edge returnEdge;
									/*Assuming the return to the depot from the former node*/
									if(adj.getEnd().getId()!=startRoute.getId()) {
										returnEdge= Optimization.inputs.getedge(adj.getEnd().getId(), endRoute.getId());
									}
									else{returnEdge= Optimization.inputs.getedge(adj.getOrigin().getId(), endRoute.getId());}
									/**/
									//Evaluo si lo puedo insertar de acuerdo al DR
									if((distance+time+returnEdge.getTime()<Optimization.inputs.getDrivingRange())) {
										insertion=true;
									}
									...
									if(insertion) {
										temporing+=time;
										operationTime+=time;
										//SI LO INSERTA DEBE INCLUIRLO EN LA LISTA DE ELEMENTOS VISITADOS (NODOS, EJES)
										visted_EdgesList.add(new Edge(adj));
										visted_nodesList.add(new Node (adj.getEnd()));
										if(adj.getEnd().getId()!=startRoute.getId()) {startRoute=new Node (adj.getEnd());}
										else{startRoute=new Node (adj.getOrigin());}
										//SI LO INSERTA LO DEBE ELIMINAR DE LA LISTA DE ELEMENTOS NO USADOS
										//adj.setState(States.Disruptions[adj.getOrigin().getId()][adj.getEnd().getId()]);
										//										Optimization.inputs.getedge(adj.getEnd().getId(), adj.getOrigin().getId()).setState(States.Disruptions[adj.getEnd().getId()][adj.getOrigin().getId()]);
										removeNode(startRoute);
										removeEdge(adj);
									}
									if(insertion) {
										sol.getRoutes().getFirst().setLastconnections(1);
										break;}
								}
							}
							/*ELSE of checking if the selected node is the depot*/
							else {insertion=false;}
							// A LA JORNADA
						}
					}	
			...
			/*DISRUPTION: DECONNECTED NODE*/
				...
				if(AdjEdges.size()!=0) {
					...
					/*Selecting the shortest path to go from the current node (startRoute) to the best edge in the network*/
					...
					if(distance+toRecourse.getTime()+best.getTime()<inputs.getDrivingRange()) {
						...
						removeEdge(best);
						/*If the best edge is disrupted it has return to the former active node*/
						if(bestDisrupted(best)) {
							Node lastconnectedNode=visted_EdgesList.get(visted_EdgesList.size()-3).getOrigin();
							Edge returnbest= inputs.getedge(best.getEnd().getId(), lastconnectedNode.getId());	
							visted_EdgesList.add(new Edge(returnbest));
							visted_nodesList.add(new Node (returnbest.getEnd()));
							removeNode(returnbest.getEnd());
							removeEdge(returnbest);
							sol.getRoutes().getFirst().setLastconnections(-1);
						}
	...
	private Edge connectionTObest(Edge ibest, Edge jbest) {
		...
		if(!isVisited(ibest) && !isVisited(jbest)) {
			...
			/*sorting by Importance after a disruption*/
			if(Optimization.aTest.getOptcriterion()==1201 ||Optimization.aTest.getOptcriterion()==1100 || Optimization.aTest.getOptcriterion()==1210) {
					if(ibest.getImportance()>jbest.getImportance()) {
					toRecourse= new Edge(ibest);}
				else {toRecourse= new Edge(jbest);}}

			/*Importance*/
			if(Optimization.aTest.getOptcriterion()==1021 ||  Optimization.aTest.getOptcriterion()==1010 ||  Optimization.aTest.getOptcriterion()==1120) {
				if(ibest.getDistance()<jbest.getDistance()) {
					toRecourse= new Edge(ibest);}
				else {toRecourse= new Edge(jbest);}}
			
			if(Optimization.aTest.getOptcriterion()==1001 || Optimization.aTest.getOptcriterion()==1012 || Optimization.aTest.getOptcriterion()==1102) {
				if(ibest.getvalueSelection()>jbest.getvalueSelection()) {
					toRecourse= new Edge(ibest);}
				else {toRecourse= new Edge(jbest);}}
		}
	...
	/*** Selecting patients directly connected with the depot ***/
	private LinkedList<Edge> patientDepotEdges(Node startRoute, Node endRoute, boolean disruption) {
		int n=Optimization.inputs.getNodes().length;
		LinkedList<Edge> AdjEdges= new LinkedList<Edge>();
		/*Selecting patient direct connected*/
		if(!disruption) {
			for(Edge e:startRoute.getAdjEdgesList()) {
				//This condition: e.getEnd().getId()!=e.getOrigin().getId() ensures that the depot is no selected because it has that same static value as the patient's one 
				if(e.getEnd().getProfit()>1 && e.getEnd().getId()!=e.getOrigin().getId()) {
					if(e.getEnd().getId()!=0 && e.getOrigin().getId()!=0) {
						if(!isVisited(e.getEnd())) {AdjEdges.add(e);}
					}
				}
			}
		}
