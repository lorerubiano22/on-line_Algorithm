...
public class InsertionAlgorithm {
	...
	public InsertionAlgorithm(Test myTest, Inputs myInputs) {
		aTest = myTest;
		inputs = myInputs;
	...
	public InsertionAlgorithm(Test myTest, Inputs myInputs, Solution solution) {
		aTest = myTest;
		inputs = myInputs;
		sol=solution;
		temporing=0;
	...
	public Solution solveMe() {
		...
		if(exploring()) {
			...
			if(visted_EdgesList.size()!=0) {
				...
				//visted_EdgesList.remove((visted_EdgesList.size()-1));
			...
			/*If this in the period of time of 1 hour*/
			if(operationTime<Optimization.inputs.DrivingRange){
				...
				if(!visted_EdgesList.isEmpty()) {
					...
					operationTime+=finalEdge.getTime();
		...
		if(promisingEdges.size()==0 || notUsedNodes.size()==0) {
	...
	private boolean importanceMainCriterion() {
		...
		if(Optimization.aTest.getOptcriterion()==1100 || 
				...
				Optimization.aTest.getOptcriterion()==1201) {critetion=true;}
		return critetion;
	}
		...
		if(!disruption) {
			for(Edge e:startRoute.getAdjEdgesList()) {
				if(visted_EdgesList.size()==0) {AdjEdges.add(e);}
					...
					//if(!isVisited(e) && !isVisited(inputs.getedge(e.getEnd().getId(), e.getOrigin().getId()))) {
					if(!isVisited(e.getOrigin()) || !isVisited(e.getEnd())) {
