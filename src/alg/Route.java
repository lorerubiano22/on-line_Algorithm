...
public class Route implements Serializable, Comparable<Route> {
	...
	public void setCenter(float[] coord) {
		center = coord;
	}
	...
	public void setLastconnections(int connection) {
		this.Lastconnections = connection;
	}
	...
	public void calcCoverage() {
		...
		for (Edge e : this.edges) {
			if (e.getEnd().getId() != 0 && e.getEnd().getProfit() > 1 && e.getEnd().getId() != 0
					&& e.getEnd().getId() != 0) {
				c++;
			}
		}
