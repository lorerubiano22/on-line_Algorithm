	...
	public static boolean solveMe(Inputs inp,double [][]TV,int [][]c,double []U,double []victims, int origin, int end) {
		...
		try {
			...
			for(int k=0;k<K;k++) {
				...
				for(int j= 1; j<n;j++) {
					expr2.addTerm(1.0, x[origin][j][k]);
				}
			...
			for(int k=0;k<K;k++) {
				IloLinearNumExpr expr2 = cplex.linearNumExpr();
				expr2.addTerm(1.0, x[origin][(end)][k]);
			...
			for(int k=0;k<K;k++) {
				for(int i= 0; i<n;i++) {
					...
					expr5.addTerm(1.0, x[i][origin][k]);
...
//			}
			...
			for(int k=0;k<K;k++) {
				...
				for(int i= 0; i<n;i++) {
					for(int j= 0; j<n;j++) {
						if(U[j]>0) {expr10.addTerm(1/K, x[i][j][k]);}
			...
			if (cplex.solve()) {
			path=true;
			System.out.println("Model solved: "+cplex.getObjValue());
			}
			else {
